...
type-scale, cliche-color (all WARN).
...
# ---------------------------------------------------------------------------
# CSS parsing: brace-depth block splitter (handles one level of nesting,
# which is all @media / @keyframes need here).
# ---------------------------------------------------------------------------
...
def parse_css(text):
    ...
    Returns (rules, custom_props, keyframe_props):
      - rules: list of {'selector': str, 'decls': [(prop, val), ...]} for
        every ordinary selector block, including those nested one level
        under @media (keyframe stops are NOT included here -- no
        contrast/spacing/etc. checks run inside @keyframes).
      - custom_props: dict of `--name` -> list of raw value strings seen in
        :root/html/body/[data-theme...] blocks (for one-level var()
        resolution; multiple different values means "ambiguous").
    ...
    return rules, custom_props, keyframe_props
...
def _contrast_threshold(decls):
    ...
    if fs:
        m = re.match(r"([\d.]+)\s*px", fs.strip(), re.IGNORECASE)
        if m:
            px = float(m.group(1))
            if px >= 24:
                threshold = 3.0
...
def _check_contrast(rule, custom_props):
    ...
    if "color" not in decls:
        return []
    bg_prop = "background-color" if "background-color" in decls else (
        "background" if "background" in decls else None
    )
    if bg_prop is None:
        return []

    color_raw = _resolve(decls["color"], custom_props)
    bg_raw = _resolve(decls[bg_prop], custom_props)
    ...
    if ratio < threshold:
        detail = (
            f"color {decls['color']} on {bg_prop}:{decls[bg_prop]} "
...
def validate_text(text, name):
    ...
    rules, custom_props, keyframe_props = parse_css(stripped)
...
def _has_checkable_css(text):
    stripped = _strip_comments(text)
    rules, _custom_props, keyframe_props = parse_css(stripped)
